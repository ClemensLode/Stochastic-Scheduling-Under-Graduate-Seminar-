...
Es sind wieder eine Reihe von Jobs gegeben, die unterschiedlich lange Zeit ben"otigen. Au"serdem hat man ein System mit diesmal zwei Prozessoren zur Verf"ugung, die beide gleich schnell arbeiten. Die einzelnen Jobs sind stochastisch unabh"angig und exponential verteilt. Das Problem ist nun wieder, eine Politik f"ur die Abarbeitung der Aufgaben zu finden, so dass die insgesamt ben"otigte Zeit minimal ist.

\begin{figure}[h]
\includegraphics[scale=0.5]{zweiprozessoren.png}
\caption{"Ubersicht Problemstellung zwei Prozessoren}
\end{figure}
...
\caption{Zustand Prozessoren mit Politik die nach Zeit zuordnet}
\end{figure}

Der zweite Versuch eine Politik aufzustellen, w"are also, die Aufgaben in zwei Reihen einzuteilen, die dann jeder Prozessor abarbeiten soll. Dies entspricht dem Problem, n Kugeln auf zwei nicht-unterscheidbare Urnen zu verteilen, wir h"atten also bis zu (n = Aufgabenzahl) \begin{displaymath}n!+(n-1)!+\cdots+2!+1!\end{displaymath} verschiedene Belegungsm"oglichkeiten f"ur die Politiken. 
...
Die Frage ist nun zuerst, ob durch unsere Regel das Problem nicht schon gel"ost wurde. Wenn wir immer eine Aufgabe auf den Prozessor legen, der nicht besch"aftigt ist, gleichen wir bei jedem Schritt beide Balken aus. Erreichen wir dadurch das optimale Gleichgewicht? Nein. Dies w"urde der Fall sein, wenn alle Aufgaben gleiche Zeit ben"otigten und eine gerade Anzahl Aufgaben vorliegt. Legen wir aber beispielsweise die Aufgaben mit aufsteigenden Zeiten auf die Prozessoren, schwankt der Unterschied zwischen beiden bei jedem Schritt mehr, 
...
Seien zwei identische Prozessoren gegeben um n Aufgaben abzuarbeiten. Beide laufen unabh"angig, k"onnen also jede Aufgabe zu jeder Zeit in einer beliebigen Reihenfolge bearbeiten. Startzeit ist t = 0. Sei im weiteren Verlauf zum Zeitpunkt 0 ein Job 0 bereits im Prozessor, d.h. starte jede Politik mit Job 0 (dies macht den Beweis etwas klarer).
Sei au"serdem \(X_{j}\) die Zeit um Aufgabe j zu bearbeiten und sei diese exponential verteilt mit \begin{displaymath}\lambda_{j} = u_{j}, j=1,\ldots,n\end{displaymath}.

F"ur jede Politik \({displaymath}\pi = (0,i_{1},\ldots, i_{n})\) ist jeweils M die Makespan und D die Zeit die einer der Prozessoren stillsteht. Somit ist zum Zeitpunkt M - D einer der Prozessoren fertig und keine weiteren Auftr"age mehr in der Warteschlange und M + M - D die Summe aller Zeiten, also \(\sum_{j=0}^n X_{j}\)
...
Kennen wir alle Wahrscheinlichkeiten \(\pi_{0}\) bis \(\pi_{n-1}\) kennen wir auch \(\pi_{n}\), da egal welche Politik wir w"ahlen \emph{auf jeden Fall} ein Job die letzte ist. Wir benutzen nun einfach die Gegenwahrscheinlichkeit \(\pi_{n} = 1 - \sum_{j=0}^{n} \pi_{j}\) TODO WARUM?
...
%\begin{figure}[h]
%\includegraphics[scale=0.5]{sap.png}
